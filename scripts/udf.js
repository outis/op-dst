	...
	const udf = globals.udf = {
		 ...
		 * @property {string} base - UDF base name
		 * @property {number} i - item index
		 * @property {string[]} names - UDF keys (parsed from DSF names, which include base & item index)
		 * @property {string[]} values - DSF values
		 *
		 *
		 * A UdfExtract expands on a {@link UdfEntry} by adding a property for DSF value and each DSF name:
		 * + the value, with key (generated by {@link this.key}) from the corresonding name
		 * + the name, with key `${key}_field`
		 *
		 * Example:
		 *
		 *     # for the item:
		 *     <li><span class="dsf dsf_skills_02_name">Crafts</span><span class="dsf dsf_foo_02">5</span></li>
		 *     # the extract is:
		 *     {
		 *         base: 'skills',
		 *         i: 2,
		 ...
		 *         values: ['Crafts', '5'],
		 *         name: 'Crafts',
		 *         name_field: 'dyn_skills_02_name',
		 *         value: '5',
		 *         value_field: 'dyn_skills_02',
		 *     }
		 *
		 *
		 * @typedef {UdfEntry} UdfExtract
		 */
		...
		postLoad() {
			module.on('postLoad', () => this.loaded = true, 'udf.postLoad');
		},
		...
		*/
		 ...
		 */
		entry(item) {
			let data = dsf.extract(item);
			return Object.entries(data).reduce((accum, property) => {
				const name = property[0],
					  value = property[1],
					  {base, i, key} = this.parseName(name);
				accum.names[key] = name;
				accum.values[key] = value;
				//accum.base ??= base;
				accum.base = base;
				//accum.i ??= i;
				accum.i = i;
		...
		*entries(list) {
		...
		/**
		 * {@link this.extractFrom Extracts} field information from a UDF item.
		 *
		 * @param {HTMLElement|selector|jQuery} item - UDF item
		 *
		 * @returns {UdfExtract}
		 */
		extract(item) {
			return this.extractFrom(dsf.extract(item));
		},

		/**
		 * {@link this.extract Extract} from all items of the give UDF.
		 *
		 * @param {} list - source of {@link this.items items}
		 *
		 * @returns {UdfExtract[]}
		 */
		extractAll(list) {
			...
			for (let item of this.items(list)) {
				items[item.name] = this.extract(item);
			}
			return items;
		},

		/**
		 * Extract from the given map.
		 *
		 * @param {DsfExtract} data - map of DSF names & values
		 *
		 * @returns {UdfExtract}
		 */
		extractFrom(data) {
		...
		/**
		 * Generate {@link this.extract extracts} of items in the given UDF.
		 *
		 ...
		 */
		*extracts(list) {
			yield* this.mapItems(list, this.extract.bind(this));
		},
...
* @param {boolean} [options.volatile] Whether to remove stored volatile values from local storage.
		...
		/**
		 * Loop over all items of all UDFs in the given section, yielding data for each.
		 *
		 ...
		 */
		scan(section) {
			// return, rather than yield*, to get tail call
			if (this.loaded) {
				return this.scanDsfs(section);
			} else {
				return this.scanDsa(section);
			}
		},

		/**
		 * Loop through the DSFs in a given section, yielding all UDF items.
		 *
		 * @param {string} section - section name
		 *
		 * @yields {UdfExtract}
		 */
		*scanDsfs(section) {
			for (let list of this.$udf(section)) {
				for (let data of this.extracts(list)) {
					yield data;
				}
			}
		},
		...
		/**
		 * Loop through the DSAs in a given section, yielding all UDF items.
		 *
		 * @param {string} section - section name
		 *
		 * @yields {UdfExtract}
		 */
		*scanDsa(section) {
			const bases = this.bases(section),
				  // TODO: determine template names from section, rather than hardcoding
				  tpls = [
					  'dyn_{base}_{i:02}',
					  'dyn_{base}_{i:02}_specialty'
				  ];
			for (let [field, value, env] of
				 dsa.entries('dyn_{base}_{i:02}_name',
							 range.keyed({
								 base: bases,
								 i:[1, this.maxCount]
							 }), {continuous:true})
			...
			{
				const names = [field, ...klass.evalAll(tpls, env)],
